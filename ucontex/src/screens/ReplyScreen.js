...
import {fetchReply, replyFormField, replyComment, resetData} from '../actions';

//const {  } = React;
class ReplyScreen extends React.PureComponent {
  constructor(props) {
    super(props);
  ...
  onSubmitReply = commentId => {
    const {userId, token, replyMsg} = this.props;
    this.props.replyComment({message: replyMsg, commentId, userId, token});
  };
  ...
  handleLoadMore = () => {
    const commentId = this.props.navigation.getParam('commentId');
    if (!this.props.isFetchingReplies) {
  ...
  renderFooter = () => {
    ...
    if (!this.props.isFetchingReplies && _.size(this.props.replyData) > 1)
      return null;
    return (
  ...
  render() {
    const {replyData, navigation} = this.props;
    ...
    return (
      <View style={{flex: 1}}>
        <Header
          ...
          <Body>
            <Text style={styles.headerTitleStyle}>Replies</Text>
          </Body>
        </Header>
          ...
          <View style={{flexDirection: 'row'}}>
            ...
            <Item style={styles.commentPanel}>
              ...
              <Input
                ...
                style={{maxHeight: 80}}
                onChangeText={text =>
                  this.props.replyFormField({
                    prop: 'replyMsg',
                    value: text,
                  })
                }
                value={this.props.replyMsg}
              />
            </Item>
            <TouchableOpacity
              onPress={() =>
                this.onSubmitReply(navigation.getParam('commentId'))
              }
...
const mapStateToProps = ({auth, replies}) => {
  //const { userData } = profileSetup;
  const {replyData, is_replying, replyMsg, isFetchingReplies} = replies;
  const {userData, token, userId} = auth;

  console.log('repl', replyData);
...
export default connect(
  mapStateToProps,
  {fetchReply, replyFormField, replyComment, resetData},
)(ReplyScreen);
