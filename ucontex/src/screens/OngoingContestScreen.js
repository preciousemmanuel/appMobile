...
class OngoingContestScreen extends Component {
  constructor(props) {
    super(props);
    this.page = 1;
  }
  ...
  renderHeaderText = () => {
    return <Text style={styles.headerTitleStyle}>Ongoing Contest</Text>;
  };

  render() {
    const {containerStyle, fabStyle} = styles;
    const {contestOngoingData, navigation} = this.props;
    return (
...
const mapStateToProps = ({contest, auth}) => {
  const {contestOngoingData, isFetchingAllContest, page} = contest;

  const {token, userId} = auth;
  console.log('ong', contestOngoingData);
  return {contestOngoingData, isFetchingAllContest, token, userId};
};
