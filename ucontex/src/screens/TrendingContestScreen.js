...
class TrendingContestScreen extends Component {
  constructor(props) {
    super(props);
    this.page = 1;
  }
  componentWillMount() {
    ...
    const {token, userId, page, fetchContest, navigation} = this.props;
    const status = navigation.getParam('status');
    fetchContest({
      token,
      userId,
      page: this.page,
      status: CONTEST_STATUS.TRENDING,
    });
  ...
  renderHeaderText = () => {
    return <Text style={styles.headerTitleStyle}>Trending Contest</Text>;
  };

  render() {
    const {containerStyle, fabStyle} = styles;
    const {contestTrendingData, navigation} = this.props;
    return (
      <View style={{flex: 1}}>
        <View>
          <Header
...
const mapStateToProps = ({contest, auth}) => {
  const {contestTrendingData, isFetchingAllContest, page} = contest;

  const {token, userId} = auth;

  return {contestTrendingData, isFetchingAllContest, token, userId};
};
export default connect(
  mapStateToProps,
  {fetchContest, resetContest},
)(TrendingContestScreen);
